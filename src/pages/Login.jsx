...
function Login({ onLogin }) {
  ...
  const handleLoginSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      if (loginEmail === 'admin@dealflow360.com' && loginPassword === 'admin123') {
        const user = { name: 'Super Admin', email: 'admin@dealflow360.com', role: 'Admin' };
        setSession(user);
        addAuditLog(user, 'User Login', 'Auth', 'Super Admin signed in');
        onLogin(user);
        toast.success('Welcome back, Super Admin!');
      } else if (foundUser) {
        const user = { name: foundUser.name, email: foundUser.email, role: foundUser.role || 'Sales Representative' };
        setSession(user);
        addAuditLog(user, 'User Login', 'Auth', `${user.name} signed in`);
        onLogin(user);
        toast.success(`Welcome back, ${user.name}!`);
      } else {
        toast.error('Invalid email or password. Try quick demo accounts below.');
        setLoading(false);
      }
    }, 400);
  };
