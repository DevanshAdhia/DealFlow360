...
export const ReportDetail = () => {
  ...
  // Resolve reportConfig with fallback for numeric or missing reportId
  const reportConfig = useMemo(() => {
    let found = REPORT_DEFINITIONS.find(r => r.id === targetId);
    if (!found) {
      const idx = parseInt(targetId, 10);
      if (!isNaN(idx) && idx >= 1 && idx <= REPORT_DEFINITIONS.length) {
        found = REPORT_DEFINITIONS[idx - 1];
      }
    }
    return found || REPORT_DEFINITIONS[0];
  }, [targetId]);
  ...
  const renderReportContent = () => {
    const activeId = reportConfig ? reportConfig.id : targetId;
    switch (activeId) {
